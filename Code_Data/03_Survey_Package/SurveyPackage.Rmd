...
```{r load data}
srs <- readRDS("../SessionData/usa_2016/srs.rds")
...
mean by svymean:
```{r svymean cl}
svymean( ~ name, design = clus_obj)
```

mean by standard mean function:
```{r mean cl}
cl %>%
  count(name, name = "votes") %>%
