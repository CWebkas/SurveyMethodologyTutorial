...
anes_prep <-
  ...
  # 2. replace predefined missing values strings with NA
  replace_with_na_all(condition = ~ .x %in% na_strings) %>%
  # 3. turn scores to numeric variables
  mutate(obama_score = as.numeric(obama_score),
         chr_fund_score = as.numeric(chr_fund_score)) %>%
  # 4. for optical reasons change labels in voting purpose
  mutate(
  ...
  # 4. turn character variables back to factors as mice (imputation) needs factors
